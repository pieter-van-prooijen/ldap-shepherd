...
(defn- paged-search-criteria
  "Returns a map of search criteria from the given base and options"
  [base options]
  (let [search-criteria (search-criteria base options)
        page (or (:page options) 0)
        page-size (or (:page-size options) 10)
        total-size (or (:total-size options) 0)
        [sort-key-name desc] (or (:sort-key options) ["cn" false])
        sort-key  (SortKey. sort-key-name desc)
        cookie (:cookie options)]
    (assoc search-criteria :page page :page-size page-size :total-size total-size :sort-key sort-key :cookie cookie)))


(defn- paged-search-results
  [connection {:keys [base scope filter attributes page page-size total-size sort-key cookie]}]
  (let [req (SearchRequest. base scope filter attributes)
        sort-control (ServerSideSortRequestControl. (into-array [sort-key]))
        vlv-req-control (VirtualListViewRequestControl. (inc (* page page-size)) 0 (dec page-size) total-size cookie)
        _ (.setControls req (into-array Control [sort-control vlv-req-control]))
        res (.search connection req)
        vlv-res-control (VirtualListViewResponseControl/get res)
        entries (when (> (.getEntryCount res) 0) (remove empty? (map entry-as-map (.getSearchEntries res))))
        total-size (when vlv-res-control (.getContentCount vlv-res-control))
        cookie (when vlv-res-control (.getContextID vlv-res-control))]
    [total-size cookie entries]))


(defn paged-search [connection base options]
  "Answer a paged and sorted search on connection. Options are the same as for search, supplemented with:

   :page  the current zero-based page of results to answer (0).
   :page-size the number of results per page (10)
   :sort-key  a tupple of [attribute desc] on which to sort the result (desc indicates if the result should be in descending order.
   :total-size the total size of the result set (returned after the first invocation of a search)
   :cookie the cookie answered after the first invocation of a search.  

   Answer a vector of [total-size cookie [results]]"
  (paged-search-results connection (paged-search-criteria base options)))

(def charset "UTF-8")
(defn encode-cookie [asn-cookie]
  (String. (base64/encode (.getValue asn-cookie)) charset))

(defn decode-cookie [s]
   (ASN1OctetString. (base64/decode (.getBytes s charset))))
